...
old_reader = '''      const p1Lines=pdf.numPages>=1?await readPageLines(pdf,1):[];
...
for token in [
    ...
    'const rows=parseClientRows(allPageLines);',
    'const installationPages=allPageLines.filter',
    'const installs=parseInstallRows(installText);',
]:
...
print('DCO multiline parser v2 applied')
